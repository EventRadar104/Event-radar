    ...
    <>
      ...
      <section style={{
        ...
        <form action="/" method="GET" style={{
          display:'flex', alignItems:'stretch', background:'var(--white)',
          border:'1.5px solid var(--border)', borderRadius:16, maxWidth:700,
          margin:'0 auto', boxShadow:'var(--shadow-md)', overflow:'hidden',
        }}>
      ...
      <div style={{ maxWidth:900, margin:'0 auto', padding:'32px 24px 80px' }}>
        ...
        {events.length === 48 && (
          <div style={{ textAlign:'center', marginTop:32 }}>
            <Link href={buildHref(params, { page: String((parseInt(params.page || '1') + 1)) })}
        ...
        {/* Teaser */}
        <div style={{ background:'var(--ink)', borderRadius:16, padding:28, color:'#fff', display:'flex', alignItems:'center', justifyContent:'space-between', gap:16, marginTop:40 }}>
          <div>
          ...
          <Link href="/trips" style={{ background:'rgba(255,255,255,.1)', color:'#fff', border:'1px solid rgba(255,255,255,.2)', borderRadius:40, padding:'10px 20px', fontSize:13, fontWeight:500, whiteSpace:'nowrap', textDecoration:'none', flexShrink:0 }}>
            Start planning →
          </Link>
        </div>
      </div>
...
function Chip({ label, href }: { label: string; href: string }) {
  return (
    <Link href={href} style={{ display:'inline-flex', alignItems:'center', gap:5, background:'var(--stone)', border:'1px solid var(--border)', borderRadius:20, padding:'4px 10px', fontSize:12, color:'var(--ink2)', textDecoration:'none' }}>
  ...
  return (
    <div style={{ textAlign:'center', padding:'60px 24px', color:'var(--ink3)' }}>
      ...
      <p style={{ fontSize:14, marginBottom:20 }}>
        {params.q ? `No results for "${params.q}" — try a broader search.` : 'Try removing some filters to see more events.'}
      </p>
