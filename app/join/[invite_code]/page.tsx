...
export default async function JoinPage({ params }: PageProps) {
  ...
  if (!info) {
    return (
      <div style={{ maxWidth: 480, margin: '80px auto', padding: '0 24px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 14, color: 'var(--ink3)' }}>This invite link may have expired or been removed.</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ maxWidth: 440, margin: '80px auto', padding: '0 24px' }}>
      <div style={{ background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 20, padding: 32, textAlign: 'center', boxShadow: 'var(--shadow)' }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--ink3)', marginBottom: 6 }}>
          {info.member_count} member{Number(info.member_count) !== 1 ? 's' : ''}
        </p>
